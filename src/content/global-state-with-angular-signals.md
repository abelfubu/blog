    ...
    new InjectionToken("appCounter", {
      ...
      factory: () => {
        ...
        return {
          ...
          increment: () => state.update((count) => count + 1),
          decrement: () => state.update((count) => count - 1),
          getCount: () => http.get<number>("https://counter.api.com").pipe(tap((count) => state.set(count))),
        };
      },
    }),
  );
...
By injecting `HttpClient`, we can now fetch the initial counter value from an external API, further demonstrating the flexibility and power of combining signals with DI.
Of course you can get creative here and inject some other deps to create a more complex state management
